...
object UserController {
  ...
  // TODO: rewrite the code to follow Julian's way of handling incoming
  // updates
  ...
  //     val newUsers =
  //       updatedUsers.filterNot(user => existingUsers.exists(_.id == user.id))
  ...
  //           println(
  //             s"Failed to delete user with ID $userId from IndexedDB. Exception: $exception"
  //           )
  //       }
  ...
  //       val message =
  //         s"User ${user.name} added! ${user.id.getOrElse("")}:${user.name}:${user.age}:${user.email}:${user.password}"
  //       TrysteroSetup.sendMessage(message)
  //     case Failure(exception) =>
  //       println(
