...
object KeyMaterialSingleton {
  ...
  private val keyMaterialPromise: Promise[KeyMaterial] = Promise[KeyMaterial]()

  def initializedKeyMaterial: Future[KeyMaterial] = keyMaterialPromise.future
  ...
  getKeyPairFromDB.onComplete {
      ...
      keyMaterial.set(
        Some(
          Ed25519KeyMaterial(
            ...
            privateKey =
              if (entry.privateKey.nonEmpty)
                Some(Base32.decode(entry.privateKey))
              else None
          )
        )
      )
      keyMaterialPromise.success(
        Ed25519KeyMaterial(
          publicKey = Base32.decode(entry.publicKey),
          privateKey =
            if (entry.privateKey.nonEmpty) Some(Base32.decode(entry.privateKey))
            else None
        )
      )
      println(
        s"Found key pair in database: Public Key: ${publicKey.now().getOrElse("None")}, Private Key: ${privateKey.now().getOrElse("None")}"
      )
    ...
    case Success(None) =>
      ...
      Ed25519KeyMaterial.create().onComplete {
        case Success(km) =>
          println(
            s"Generated new key pair: Public Key: ${Base32.encode(km.publicKey)}}, Private Key: ${km.privateKey.map(Base32.encode).getOrElse("None")}"
          )
          ...
          keyPairTable.add(entry).toFuture.onComplete {
            ...
            case Failure(e) =>
              println(s"Failed to add key pair to database: ${e.getMessage}")
          }
    ...
    case Failure(exception) =>
      println(
        s"Error retrieving key pair from database: ${exception.getMessage}"
      )
  }
}
